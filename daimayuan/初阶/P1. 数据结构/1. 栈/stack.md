...
从栈顶往下数：`第1个元素的位置是top`，`第2个元素的位置是top-1`...以此推类。所以确定了符号`-`，
>要使得`top-k = top`，**观察`top`和`k`之间的变量关系**则可以得出：`第k个元素的位置则是top-k+1`。
>>>>>![](https://pic.imgdb.cn/item/6486f5491ddac507cce92d42.jpg)
